...
# Function to parse the raw text into a list of tuples
def parse_vocabulary(text):
    ...
    for line in lines:
        if '=' in line:
            ...
            vocabulary.append((word, definition))
    return vocabulary

vocabulary = parse_vocabulary(raw_text)

# Ensure vocabulary is parsed correctly
if not isinstance(vocabulary, list):
    raise TypeError("Vocabulary must be a list of tuples")

# Step 2: Construct the HMM
states = [word for word, _ in vocabulary]
definitions = {word: definition for word, definition in vocabulary}
n_states = len(states)

# Create more realistic transition probabilities
...
start_probability = np.ones(n_states) / n_states

# Recursive function for HMM
def generate_sentence(current_state, depth=0, max_depth=10):
    if depth >= max_depth:
        return [current_state]
    next_state = np.random.choice(states, p=transition_matrix[states.index(current_state)])
    return [current_state] + generate_sentence(next_state, depth + 1, max_depth)

# Generate sentences in EdgeLore language
def generate_edgelore_sentence(max_depth=10):
    start_state = np.random.choice(states, p=start_probability)
    sentence = generate_sentence(start_state, max_depth=max_depth)
    return ' '.join([definitions[word] for word in sentence])

# Interactive function
def interact_with_model():
    print("Welcome to the EdgeLore Language Model!")
    print("Type a message and the model will respond in the EdgeLore language.")
    ...
    while True:
        ...
        if user_input.lower() == 'exit':
            ...
            response = generate_edgelore_sentence()
