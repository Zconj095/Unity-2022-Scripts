...
# Load templates
menu_template = cv2.imread('GameMenu.png', 0)
game_template = cv2.imread('NoMenu.png', 0)
...
def get_game_state():
    ...
    # Template matching
    menu_match = cv2.matchTemplate(screenshot_gray, menu_template, cv2.TM_CCOEFF_NORMED)
    game_match = cv2.matchTemplate(screenshot_gray, game_template, cv2.TM_CCOEFF_NORMED)

    threshold = 0.8
    if np.max(menu_match) > threshold:
        return 'menu'
    elif np.max(game_match) > threshold:
        return 'game'
    else:
        return 'unknown'
